...
function DataGridRowImpl<TData>({
    ...
    return (
        ...
        >
            {rowDragHandle && (
                <div className="absolute inset-y-0 inset-s-0 z-20 flex items-center px-1">
                    {rowDragHandle}
                </div>
            )}
            ...
            {visibleCells.map((cell, colIndex) => {
                ...
                const isCellSelected =
                    cellSelectionKeys?.has(
                        getCellKey(virtualRowIndex, columnId),
                    ) ?? false;

                const isSearchMatch =
                    searchMatchColumns?.has(columnId) ?? false;
